...
for k=0:10
    for i=1:number
       for j=1+i:number
           ...
           if Sumpcc(i)>Sumpcc(j)-s && ecc(i)>ecc(j)-s   
        walkPF(i,k+1)=walkPF(i,k+1)+(Sumpcc(i)-Sumpcc(j))*k/10+(ecc(i)-ecc(j))*(1-k/10);
           end
           if  Sumpcc(j)>Sumpcc(i)-s && ecc(j)>ecc(i)-s
...
Top100_TP=importdata('Model analysis-PeC_W/SPPI6099_gene.csv');
...
Top100_ne=importdata('Model analysis-PeC_W/SPPI6099_no_essentialgene.csv');
...
EPPI_rank=readtable('Model analysis-PeC_W/SPPI6099_rank.csv');
...
writetable(list2, 'Model analysis-PeC_W/SPPI6099_result.csv');
