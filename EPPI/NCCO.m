...
mkdir NCCOdata; %create fold storing data files
...
for i=1:11
  [value1(:,i),inx1(:,i)] = sort(PF(:,i),'descend'); %按分值按降序排列
end
for j=1:11
  for i = 1:6
...
writecell(Top100_TP, 'Model analysis-NCCO_W/EPPI_gene.csv');
Top100_ne=setdiff(Top100,Top100_TP);
writecell(Top100_ne, 'Model analysis-NCCO_W/EPPI_no_essentialgene.csv');
for i=1:number
    rank(i,1)=i;
end
Topall=Pro(inx1(1:number,1));
Topall_rank=table(Topall,rank);
writetable(Topall_rank, 'Model analysis-NCCO_W/EPPI_rank.csv');
