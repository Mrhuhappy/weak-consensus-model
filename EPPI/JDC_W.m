...
%Binarized gene expression Matrix
Matrix1=zeros(number,9);
for i=1:number
    for j=1:9
        if  Matrix(i,j)>G(i,1)
            Matrix1(i,j)=1;
...
for k=0:10
    for i=1:number
       for j=1+i:number
         ...
         if JD(i)>JD(j)-s/10 && ecc(i)>ecc(j)-s/10
       walkPF(i,k+1)=walkPF(i,k+1)+(JD(i)-JD(j))*k/10+(ecc(i)-ecc(j))*(1-k/10);  
         
           end
           if  JD(j)>JD(i)-s/10 && ecc(j)>ecc(i)-s/10
...
Top100_TP=importdata('Model analysis-JDC_W/EPPI_gene.csv');
...
Top100_ne=importdata('Model analysis-JDC_W/EPPI_no_essentialgene.csv');
...
EPPI_rank=readtable('Model analysis-JDC_W/EPPI_rank.csv');
...
writetable(list2, 'Model analysis-JDC_W/EPPI_result.csv');
