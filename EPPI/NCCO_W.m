...
for k=0:10
   for i=1:number
      for j=i+1:number
           if NCC(i,1)>NCC(j,1)-s/10 && OS(i,1)>OS(j,1)-s/10  
               PF(i,k+1)=PF(i,k+1)+(NCC(i,1)-NCC(j,1))*k/10+(OS(i,1)-OS(j,1))*(1-k/10);
               
           end
           if NCC(i,1)-s/10<NCC(j,1) && OS(i,1)-s/10<OS(j,1)  
...
Top100_TP=importdata('Model analysis-NCCO_W/EPPI_gene.csv');
weak_Topall=Pro(inx1(1:number,11));
for i=1:number
    NCCO_W_rank(i,1)=i;
end
list=table(weak_Topall,NCCO_W_rank);
weak_Top100=Pro(inx1(1:100,11));
...
Top100_ne=importdata('Model analysis-NCCO_W/EPPI_no_essentialgene.csv');
...
EPPI_rank=readtable('Model analysis-NCCO_W/EPPI_rank.csv');
...
writetable(list2, 'Model analysis-NCCO_W/EPPI_result.csv');
